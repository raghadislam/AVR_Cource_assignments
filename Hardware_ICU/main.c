...
int main()
{
	...
	/* generate pwm signal with 50% Duty Cycle from Timer0 and period = 256 micro sec (FASTPWM)*/
	...
	while(1)
	{
		...
		ICU_enuGetOnTime_us(&Local_u8ONTime);
		
		/* print results */
		LCD_enuGoto(1,0);
		LCD_enuWriteNumber(Local_u8DutyCycle);
		LCD_enuGoto(2,0);
		LCD_enuWriteNumber(Local_u8ONTime);
	}
	return 0;
}
