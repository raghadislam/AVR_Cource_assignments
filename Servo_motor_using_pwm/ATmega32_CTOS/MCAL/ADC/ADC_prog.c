...
static u8 Global_u8ADCState = ADC_IDLE;
...
ES_t ADC_enuBlockingRead(u8 Copy_u8Channel, u16* Copy_pu16Reading){
	...
	if(Copy_pu16Reading != NULL){
		...
		if(Global_u8ADCState == ADC_IDLE) {

			/* ADC now is busy */
			Global_u8ADCState = ADC_BUSY;
			...
			while((Get_bit(ADCSRA,ADCSRA_ADIF) == ADC_LOW) && (Local_u32TimeCount < ADC_u32TIMOUT)){
				Local_u32TimeCount++;
			}
